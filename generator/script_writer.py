...
2. **按 6 大板块组织**，每个板块有 Markdown 二级标题
3. **每个板块内按子类型分四段**：
   - `### 🔮 新机爆料`：尚未发布/确认的新品传闻、泄露、专利、曝光
   - `### 🆕 新机发售`：已发布、开售、预定中的新品
   - `### 📱 系统更新`：固件、系统、驱动、CFW、模拟器等软件版本更新
   - `### 📋 其他资讯`：评测、配件、社区讨论等不属于以上三类的内容
   - 如果某个子类型没有新闻，可以跳过不写
4. **每个板块总共约 10 条资讯**（最多 10 条，如果某板块不足 10 条有几条写几条，不凑数）
5. **每条资讯格式**：
   ```
   #### [序号]. [简洁吸引人的标题]
   日期: [日期] | 来源: [来源名]
...
9. **重要**：如果新闻数据中有 `sub_type` 字段（值为 leak/release/system/general），请严格按照这个分类将新闻归入对应的子板块。没有 sub_type 的新闻归入 📋 其他资讯。
